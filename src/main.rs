...
const DESCRIBE: &str = r#"{
  ...
  "provenance_limitations": [
    ...
    "REGRESSED AT THE CURRENT PIN, and this is the honest number: at 7d490b8ecd357199c0c0e9f3e32becd5eb507c34, `detailed_placement` matches the reference on 18 of 28 comparable cases. It matched 28 of 28 at the previous pin 945a9f48dc6e5cc91d865daa92c45a1094cb682c, sweep-level -- upstream's per-iteration debug trace and this engine's matched line for line. A SCORE IS ONLY TRUE OF ONE COMMIT.",
    "The regression is UPSTREAM MOVING, not a defect introduced here: OpenROAD reworked the negotiation legalizer's INITIAL SNAPPING across 7 commits and refreshed 24 of its own goldens (`6578f2b9b1`). All 10 failing cases are exactly the comparable cases whose golden changed, and no other case moved. Three named mechanisms are not yet transcribed: `Grid::gridRoundX` (initial x now ROUNDS to the nearest site where it truncated), `displacementInSites`/`rowDispInSites` replacing `NegCell::displacement()` so displacement is measured in site widths on BOTH axes, and the new `initialSnap()` diamond search.",
