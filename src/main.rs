...
const DESCRIBE: &str = r#"{
  ...
  "commands": [
    ...
    {
      "name": "remove-fillers",
      "summary": "destroy every filler instance (an unlocked CORE_SPACER)",
      "args_template": ["remove-fillers", "{odb}"],
      "optional": [
...
/// `remove_fillers` — destroy every filler instance (`isFiller`: an unlocked CORE_SPACER).
fn remove_fillers(args: &[String]) -> ExitCode {
    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            a if odb.is_none() => odb = Some(a.to_string()),
            a => {
                eprintln!("vyges-dpl: remove-fillers: unexpected argument {a}\n\n{USAGE}");
                return ExitCode::from(2);
            }
        }
        i += 1;
    ...
    let Some(path) = odb else {
        eprintln!("vyges-dpl: remove-fillers needs <design.odb>\n\n{USAGE}");
        return ExitCode::from(2);
    };
    let mut db = match Db::open(&path) {
        Ok(d) => d,
    ...
    let removed = match vyges_dpl::fillers::remove_fillers(&mut db) {
        Ok(r) => r,
